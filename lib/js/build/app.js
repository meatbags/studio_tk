...
;var App = {
		...
		if (isHome) {
			$.ajax({
				...
				success: function(data){
					const body = '<div id="body-mock">' + data.replace(/^[\s\S]*<body.*?>|<\/body>[\s\S]*$/ig, '') + '</div>';
					const $body = $(body);
					for (let i=0; i<$body[0].children.length; i+=1) {
						const node = $body[0].children[i];
						if (node.innerHTML.indexOf('window._sharedData') != -1) {
							let text = node.innerHTML.split('=');
							text.shift();
							text = text.join('=');
							text = text.replace(/;/g, '');
							const feed = JSON.parse(text);
							let images = feed.entry_data.ProfilePage[0].user.media.nodes;
							images = images.map(function(item){
								console.log(item);
								return item.thumbnail_src;
							})
							for (let j=0; j<images.length; j++) {
								$('.instagram-grid').append(
									$('<div />', {
										class:'grid__sixth responsive item',
										html:'<div class="item__inner"><a target="_blank" href="https://www.instagram.com/teuberkohlhoff/"><img src="' + images[j] + '" /></a></div>'
									})
								);
							}
						}
	...
	initIndexPage: function() {
		...
		if (window.location.hash) {
			$('.index-trigger').each(function(i, e){
				const ref = $(e).find('a')[0].href;
				if (ref.indexOf(window.location.hash) !== -1) {
					$(e).click()
				}
			})
		}
	...
	loadingBar: function() {
		...
		$('.to-load').each(function(i, e){
			console.log(e.complete, e.naturalWidth);
		});
